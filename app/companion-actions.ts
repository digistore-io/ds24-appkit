...
import {
  ...
  type CompanionTurn,
} from "@/lib/ai/companion-rules";
import { companionById } from "@/lib/ai/companions";
import {
  companionOffReason,
  isCompanionEnabled,
  type CompanionOffReason,
} from "@/lib/ai/companion-switch";
 ...
 *
 * `CompanionInput.ask` is appended after the fence, so anything placed there is
 ...
 * `components/companion-panel.tsx` translates through the `errors` namespace.
 */
export async function askCompanionAction(input: {
  ...
  try {
    const result = await askCompanion({
      ...
      // is the fenced field: `buildCompanionRequest` wraps every entry in
      // `<customer-text …>` and neutralises the markers, and the layer's
      // standing rule names that tag as content rather than instruction. `ask`
      // is appended after the fence and is therefore app-authored by
      // construction — putting a customer's sentence there would hand the one
      // string an attacker fully controls to the model as an instruction.
      ...
      // `CompanionInput` states, and this is the caller it was written for.
      history,
      memberId,
    });
    answer = result.text;
