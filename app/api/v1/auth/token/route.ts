...
import { createKey } from "@/lib/api-keys/keys";
import {
  ApiKeyError,
  checkKeyName,
  isLifetime,
  ...
  type LifetimeDays,
  type Scope,
} from "@/lib/api-keys/rules";
import { isApiEnabled } from "@/lib/api/config";
import { TOKEN_MINT_BUCKET, TOKEN_MINT_LIMIT, apiError, apiJson } from "@/lib/api/rules";
...
export async function POST(request: Request): Promise<Response> {
  ...
  // 2. Is the feature on? 404, the shipped state — see lib/api/guard.ts.
  if (!isApiEnabled()) {
  ...
  try {
    ...
    if (error instanceof ApiKeyError && error.code === "tooManyKeys") {
      return apiError(
        "badRequest",
