...
| Is there an idea? | `docs/product-brief.md` | missing and the user cannot say in two sentences what the app does → **`market-research`**. The minimal brief `build-app` step 0 writes counts — presence answers this row, with or without the research labels |
...
| Who authors the content? | a `Content authority:` line in `docs/app.md`; failing that, the decisions section there | no line, and the app carries content tables plus an admin CRUD only the operator ever edits → the fork in `docs/content-authority.md` was never decided. For a LIVE app, moving content into code is a migration — plan it, do not just delete tables. **A recorded answer is an answer** — say so and move on |
...
| Is payment connected? | `DIGISTORE_API_KEY` in `.env`; ids on the products in `config/digistore-products.json` (`productIds.<env>`; older apps: `productIdByLanguage`); `DIGISTORE_IPN_PASSPHRASE` + `DIGISTORE_IPN_DOMAIN_ID` | key but no ids → the sync never ran; no passphrase → no IPN, so purchases arrive nowhere → **`setup-digistore`**. A product id for only *some* of the app's languages → the missing ones get an order form in the wrong language → re-run `node run.mjs ds24-sync` and read its warnings. Ids only under `dev` on an app about to launch → the PROD set does not exist yet → **`go-live`** |
