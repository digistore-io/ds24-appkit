...
//
// The imperative shell around `lib/api-keys/rules.ts`: this file owns the
...
//
// One table, two audiences (`mcp` for AI clients, `api` for the customer's
// own programs). Every function here takes the audience it acts for, and
// `authenticate()` refuses a key across the line — an `mcp` key never opens
// `/api/v1`, an `api` key never opens the MCP endpoint. See
// `db/schema-api-keys.ts` for why.
//
...
// a SHA-256 and there is nothing to return — see `db/schema-api-keys.ts` for
 ...
 * first.
 *
 * Throws `ApiKeyError("tooManyKeys")` at the limit. Revoked and expired keys
 * do not count, so replacing a key never hits it; the limit is per audience,
 * so a full MCP card never blocks the API card.
 */
export async function createKey(args: {
  ...
  if (live >= MAX_LIVE_KEYS) throw new ApiKeyError("tooManyKeys");
 ...
 *
 * Throws `ApiKeyError("unknownKey")` when nothing matched, which covers both
...
export async function revokeKey(args: { memberId: string; keyId: string }): Promise<void> {
  ...
  if (!row) {
    ...
    const [existing] = await db
      ...
      .limit(1);
    if (!existing) throw new ApiKeyError("unknownKey");
  }
}
 ...
 * even if a prefix ever lied. An `mcp` key on `/api/v1` is `malformed`, not
...
/** Written at most once a minute — see `db/schema-api-keys.ts`. */
