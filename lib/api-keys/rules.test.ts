...
describe("checkKeyName", () => {
  ...
  it("refuses a blank name rather than defaulting one", () => {
    ...
    expect(checkKeyName("   ")).toEqual({ ok: false, code: "nameRequired" });
    expect(checkKeyName("")).toEqual({ ok: false, code: "nameRequired" });
    expect(checkKeyName(undefined)).toEqual({ ok: false, code: "nameRequired" });
    expect(checkKeyName(42)).toEqual({ ok: false, code: "nameRequired" });
  });
