...
//
// Shared by `app/api/mcp/route.ts` and `lib/api/guard.ts` — the same three
// questions in front of both surfaces, so a fix lands once. Pure functions of
// a request (plus `APP_URL`), no database, no side effects.
 ...
 * own machine. A real native client — an MCP client, a mobile app — sends no
