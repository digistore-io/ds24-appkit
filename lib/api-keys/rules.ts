...
//
// One key system, two audiences: the MCP endpoint (`docs/mcp.md`) and the
// HTTP API (`docs/api.md`) hand out the same credential with the same
// lifecycle, so the rules live once. Everything here is testable without a
// database and without a request — the same split `lib/tokens/rules.ts` and
// `lib/users/rules.ts` make, for the same reason: what governs access to
...
// (AD-10). The MCP and API endpoints are the one exception, and deliberately:
// their caller is a program, not a person, so their errors are English and
// stable.

// ── The key format ──────────────────────────────────────────────────────────

/** Which surface a key opens. A key never crosses over — see `keys.ts`. */
export const AUDIENCES = ["mcp", "api"] as const;
 ...
 * what they are looking at. Two prefixes, because the prefix is also how a
 * human tells "the key for my AI client" from "the key inside my mobile app"
 * — and how `looksLikeKey` refuses one on the other's endpoint before any
 * database is asked.
 */
 ...
 * the remaining 232 is not a thing that happens. Both markers are eight
 * characters, so one length serves both audiences.
 */
export const PREFIX_LENGTH = KEY_PREFIXES.mcp.length + 4;
 ...
 * is part of the shape: an `mcp` key on the API endpoint is refused HERE,
 * before any query. It proves nothing about the key being valid; that is
 * `keys.ts`.
 */
 ...
 *
 * `mcpDisabled` / `apiDisabled` are here rather than in the two features'
 * own modules because they surface in the same place as the rest — the key
 * cards on `/dashboard/account` — and one registry entry beats three.
 */
