...
// Three separate places need the answer, and only one of them can import
// TypeScript:
//
//   scripts/ai/check.mjs   — the hint "you pay for a model and use it only for
//                            support". A `scripts/` command, plain Node.
//   lib/ai/disclosure.mjs  — "switched on and undisclosed", read by
//                            `node run.mjs legal-check`, which has no `needs`
//                            deliberately and therefore cannot import a `.ts`.
//   the app                — whether to render a panel at all.
//
// Three copies of one predicate over one file have a named failure mode, and it
// is quiet: the first rename of `config/ai-companion.json`, or the first app
// that writes `"enabled": "true"`, makes each copy answer differently — and the
// loudest symptom is `ai-check` telling every installed app it has no
// product-side call again, with no test anywhere going red.
//
// So the predicate lives here and the other three import it. This is not a new
...
// is `lib/ai/companion-switch.ts`, and `switch` is the word AC 9 itself uses.
