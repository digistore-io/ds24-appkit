...
export function priceFor(table, provider, model) {
  ...
  const hasTokens = Number.isFinite(rawInput) && Number.isFinite(rawOutput);
  const hasImage = Number.isFinite(image) && image >= 0;
  if (!hasTokens && !hasImage) return null;

  // Coerced BEFORE the fallbacks below use them. `cachedInput` falls back to
  // `input`, so an entry priced per picture only would otherwise carry NaN
  // into every term of the sum and produce a cost of NaN — which reaches the
  // database as a number that is not one.
  const input = hasTokens ? rawInput : 0;
  const output = hasTokens ? rawOutput : 0;
