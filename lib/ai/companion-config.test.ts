...
import { companionConfigFrom, isCompanionEnabled } from "./companion-config.mjs";
import { companionProblems } from "./companion-switch";
...
describe("the switch has ONE reader, and the tree says so", () => {
  ...
  it("has every consumer import companion-config.mjs", () => {
    for (const file of CONSUMERS) {
      expect(read(file), file).toContain("companion-config.mjs");
    }
  });
  ...
  it("knows that disclosure.mjs keeps the CHAT comparison, and why", () => {
    // Recorded so the exception is deliberate rather than rediscovered: the one
    // `enabled === true` left in a consumer is the assistant's, and the
    // assistant has no shared reader.
    const code = codeOf("lib/ai/disclosure.mjs");
  ...
  it("strips comments without stripping the code — the guard's own guard", () => {
    ...
    for (const file of CONSUMERS) {
      ...
      expect(codeOf(file), file).toContain("companion-config.mjs");
    }
  });

  it("keeps the comparison in the module that owns it", () => {
    expect(read("lib/ai/companion-config.mjs")).toMatch(/enabled === true/);
  });
});
