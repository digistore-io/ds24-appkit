 ...
 */
export function resolveProvider(provider, configuredProviders = [], kind = "text") {
  ...
  const anyConfigured = PROVIDER_IDS.find((id) => configuredProviders.includes(id));
  if (anyConfigured && providerCan(anyConfigured, kind)) return anyConfigured;
...
export function resolveBinding(config, task, configuredProviders = []) {
  ...
  const provider = resolveProvider(merged.provider, configuredProviders, kindOfTask(task));
  ...
  const usePinned = pinned !== "" && pinned !== AUTO && merged.provider !== AUTO;
  ...
  const defaults =
    kindOfTask(task) === "image" ? PROVIDER_DEFAULT_IMAGE_MODELS : PROVIDER_DEFAULT_MODELS;

  return {
    ...merged,
    provider,
    model: usePinned ? pinned : (defaults[provider] ?? null),
  };
}
...
export function bindingProblems(config, configuredProviders) {
  ...
  for (const task of TASKS) {
    ...
    const pinned = typeof declared.model === "string" ? declared.model.trim() : "";
      ...
      problems.push(
        isAuto
          ? `Task "${task}" needs a provider that can produce ${kind}, and the key on this ` +
              `machine is ${binding.provider}, which cannot. ${able.length === 0
          ...
          : `config/ai-models.json → ${where}.provider: "${binding.provider}" cannot produce ` +
              `${kind}, and task "${task}" is a ${kind} task. ${able.length === 0
                ? "No provider in this template can."
    ...
    if (!configuredProviders.includes(binding.provider)) {
      ...
      const haveButCannot =
        isAuto && configuredProviders.length > 0 && !configuredProviders.some((id) => providerCan(id, kind));
        ...
        haveButCannot
          ...
          : isAuto
            ? `Task "${task}" runs on "${AUTO}", and this machine has no provider key at all. ` +
                `Add ONE of these to .env: ${(kind === "text" ? PROVIDER_IDS : able)
                  .map((id) => PROVIDER_ENV_VARS[id])
                  .join(", ")}.`
