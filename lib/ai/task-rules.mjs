 ...
 */
export function resolveProvider(provider, configuredProviders = []) {
  if (PROVIDER_IDS.includes(provider)) return provider;
  return PROVIDER_IDS.find((id) => configuredProviders.includes(id)) ?? LAST_RESORT_PROVIDER;
}
...
export function resolveBinding(config, task, configuredProviders = []) {
  ...
  const provider = resolveProvider(merged.provider, configuredProviders);
  ...
  return {
    ...merged,
    provider,
    model: usePinned ? pinned : PROVIDER_DEFAULT_MODELS[provider],
  };
}
      ...
      problems.push(
        isAuto
          ? `Task "${task}" runs on "${AUTO}", and this machine has no provider key at all. ` +
              `Add ONE of these to .env: ${PROVIDER_IDS.map((id) => PROVIDER_ENV_VARS[id]).join(", ")}.`
