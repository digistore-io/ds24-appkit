...
// predicate lives in `lib/ai/companion-config.mjs`, because `scripts/ai/check.mjs`
...
// `import … from "./companion-config"` resolve to the `.mjs` at runtime while the
...
import { companionConfigFrom, isCompanionEnabled as enabledFrom } from "./companion-config.mjs";
import {
  MAX_COMPANION_HISTORY_TURNS,
  MAX_COMPANION_INPUT_CHARS,
  companionHistoryTurns,
  companionInputChars,
} from "./companion-rules";
