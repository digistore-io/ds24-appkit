...
//
// The same shape as `lib/mcp/config.ts`, for the same reason: one switch, a
// property of the PRODUCT. The API calls nothing outward, so there is no
...
//
// ── It ships OFF, like the MCP server ──────────────────────────────────────
...
// `lib/billing-mode.ts` and `lib/mcp/config.ts` follow.
 ...
 *
 * `lib/api/config.test.ts` fails the build on a non-empty result. The point is
...
export function apiConfigProblems(): string[] {
  ...
  if (config.requiresPlan !== null) {
    ...
    if (!plan) {
      ...
      // The same refusal `mcp.json` gets, and for the same reason: a balance
      // is not an entitlement, so `hasPlan()` answers false for a token
