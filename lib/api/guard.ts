...
import { mayRun, type Scope } from "@/lib/api-keys/rules";
import { apiConfig, isApiEnabled } from "@/lib/api/config";
import {
 ...
 * read-only key. The refusal lives here, in the call path, for the same reason
 * the MCP route checks scope on `tools/call` and not in `tools/list`: hiding
 * an endpoint is cosmetics, the refusal has to be in the call.
 *
...
export async function guardApi(
  ...
  //    exist, exactly like the MCP endpoint.
  if (!isApiEnabled()) {
