...
// error codes are English and stable, exactly like the MCP endpoint's and the
// opposite of every member-facing rules layer. They are deliberately NOT
...
// accepted — the same trade the sign-in and MCP limits already make.
 ...
 * pace — a runaway brake, not a pricing lever. The same figure as the MCP
 * endpoint's, deliberately: one member, one ceiling per surface.
 */
 ...
 * the per-key view cannot see. Same shape as `lib/mcp/rules.ts` and the
 * password sprint limit in `lib/credentials/rules.ts`.
 */
