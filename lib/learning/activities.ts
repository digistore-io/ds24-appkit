...
// deliberately that model rather than the other two registries: `lib/mcp/tools.ts`
// ships three examples to replace and `lib/cron/jobs.ts` ships five real jobs,
// but an activity the template put in front of a vendor's own customers would
// be an activity nobody chose. So this ships **empty**, and a page renders
