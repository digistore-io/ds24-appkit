...
// written by the server (`lib/learning/results.ts` → the activity's `grade()`);
...
export const activityResults = pgTable(
  "activity_results",
  {
    ...
    // Which registry entry (`lib/learning/activities.ts`) this row belongs to.
    ...
    // not count one — see `lib/learning/rules.ts` → `applyVerdict`.
