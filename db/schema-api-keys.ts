...
// key travels in an `Authorization: Bearer …` header. Two audiences share the
// table because they are the same credential with the same lifecycle:
//
//   `mcp` — an AI client (Claude Code, Claude Desktop, claude.ai) calling the
//           MCP endpoint. See `docs/mcp.md`.
//   `api` — a program of the customer's own — typically their mobile app —
//           calling `/api/v1/…`. See `docs/api.md`.
//
// A key is BOUND to its audience: `authenticate()` in `lib/api-keys/keys.ts`
// refuses an `mcp` key on the API and an `api` key on the MCP endpoint. The
// point is blast radius — a key pasted into an AI client must not silently
// double as a full REST credential, and vice versa.
//
...
// `lib/api-keys/rules.ts` → `keyState()` is what turns these three timestamps
 ...
 * has to be in the call (`app/api/mcp/route.ts` against `readOnly`,
 * `lib/api/guard.ts` against the handler's scope requirement).
 *
 * The Postgres enum is still called `mcp_scope` — it predates the second
 * audience, and renaming a pg enum buys nothing while adding migration risk.
 */
export const keyScopeEnum = pgEnum("mcp_scope", ["read", "write"]);

/**
 * Which surface a key opens. See the header — a key never crosses over.
 * Existing rows predate the column and were all MCP keys, hence the default.
 */
...
export const apiKeys = pgTable(
  "api_keys",
  {
    ...
    audience: keyAudienceEnum("audience").notNull().default("mcp"),
    createdAt: timestamp("created_at").notNull().defaultNow(),
    // Last successful authentication. Written at most once a minute (see
    // lib/api-keys/keys.ts) — an exact value would mean a write on every
